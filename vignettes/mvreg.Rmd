...
Once a `mvreg` model is fitted, you can obtain confidence intervals for both $\hat{\boldsymbol{\beta}}$ and $\hat{\boldsymbol{\tau}}$ using the generic function `confint()`.
...
```
If `parm` is not specified, confidence intervals for all parameters will be returned. You can also limit the output by specifying the `parm` argument, which accepts either a character vector with the parameter names or a numeric vector with the indices of the parameters. For instance:
...
If two or more models are provided, they are ranked by the number of parameters in increasing order, and comparisons are made sequentially between each pair of models.

### `anova()` with just one model
For the mean component, the function fits all possible nested models, using the full `formula.s2` as specified in the input model. The comparison begins with the intercept-only model, incrementally adding variables one by one from left to right as they appear in the original `formula.mu`. For each comparison (indexed by $j$), the model with the first $k$ variables is compared to the model with the first $k + 1$ variables. These comparisons are performed using likelihood ratio tests of the form:
...
If two or more models are passed to `anova()`, the function first ranks them in increasing order based on the number of parameters and then proceeds to sequentially test them in pairs via a likelihood ratio test.
...
The output of `anova()` when two or more models are passed consists of one table where the models are listed by row, starting from the one with fewer parameters to the one with more. Minus two log-likelihood (`-2logLik`), number of parameters (`n.param`), Akaike information criterion (`AIC`), and residual degrees of freedom (`df.residual`) are printed for all models. Additionally, the likelihood ratio test statistic (`LRT`), its degrees of freedom (`df`), and p-value (`Pr(>Chi)`) are printed in the rows where a model is compared to the one preceding it.

Information about the model formulas being compared is printed before the table.


### Some notes on the use of `anova()` function

The comparison between models via likelihood ratio test is only possible where the models are nested. If the user passes to `anova()` function two `mvreg` models with the same number of parameters, the p-value is not printed since it is sure that the two models cannot be nested, as the degrees of freedom of the test is equal to 0. For example

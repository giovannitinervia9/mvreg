...
The expressions for the first and second derivatives of the log-likelihood are
...
A closed form expression for $\hat{\boldsymbol{\beta}}$ is the weighted least squares (WLS) estimator $\hat{\boldsymbol{\beta}} = (\mathbf{X}'\mathbf{W}\mathbf{X})^{-1}\mathbf{X}'\mathbf{W}\mathbf{y}$, in which $\mathbf{W}$ is a diagonal matrix with elements $w_{ij} = \dfrac{1}{\exp\left\{\mathbf{z}_i'\hat{\boldsymbol{\tau}}\right\}} \text{ } \forall i = j$.

There is no closed-form solution for $\hat{\boldsymbol{\tau}}$, so the Newton-Raphson method is used to estimate it.

### Starting values
Starting values for the parameters are provided by the function `mvreg_start()`, which takes as input the response vector `y`, the design matrices `x` and `z` and a method for generating the initial guess for $\hat{\boldsymbol{\tau}}_0$ via the argument `start.s2`.
...
A second option is `start.s2 = "gamma"`, in which the squares of the empirical residuals are regressed on $\mathbf{Z}$ using Gamma generalized linear model.

The last and simpler option is `start.s2 = "zero"`, in which the first guess is simply given by $\hat{\boldsymbol{\tau}}_0 = (\log\hat{S}^2, 0, 0, \ldots, 0)_p'$.

### Estimation algorithm
The algorithm for fitting the parameters via maximum likelihood is contained in the `mvreg_fit()` function, which takes in input the response vector `y`, the design matrices `x` and `z` and the starting values `b0` and `t0` provided by `mvreg_start()`.
...
Next, the weights $w_i$ are estimated via $w_i = \dfrac{1}{\exp\left\{\mathbf{z}_i'\hat{\boldsymbol{\tau}_{k}}\right\}}$ and used to to compute $$\hat{\boldsymbol{\beta}}_k = (\mathbf{X}'\mathbf{W}\mathbf{X})^{-1}\mathbf{X}'\mathbf{W}\mathbf{y}$$

The algorithm continues iterating until the convergence criterion $|(\hat{\boldsymbol{\beta}},\hat{\boldsymbol{\tau}})_k - (\hat{\boldsymbol{\beta}},\hat{\boldsymbol{\tau}})_{k-1}| < \boldsymbol{\varepsilon}$ is satisfied.

This algorithm is the default choice for `mvreg_fit()`, but an argument `method` is available. If `method = "wls"` the algorithm described above is used. If `method = "full_nr"` the closed form expression for $\hat{\boldsymbol{\beta}}$ is not used and the Newton-Raphson method is applied to both the mean and variance components.
...
The variance-covariance matrix is computed by `mvreg_fit()` function, that has a `vcov.type` argument. By default `vcov.type = "expected"`, meaning that the variance-covariance matrix is computed using the expected Fisher information matrix $\mathcal{I}^{-1}(\hat{\boldsymbol{\beta}}, \hat{\boldsymbol{\tau}})$. Alternatively, the user can specify to use the observed Fisher information matrix $\mathcal{J}^{-1}(\hat{\boldsymbol{\beta}}, \hat{\boldsymbol{\tau}})$.

Once a `mvreg` model is fitted, the variance-covariance matrix can be accessed using `vcov()` method.  
The `vcov` method for `mvreg` objects also has a `partition` argument, which allows the user to specify which diagonal block of the full matrix should be returned. The default is `"all`", but `"mu"` (for the mean component) and `"s2"` (for the variance component) can also be selected.
...
Once $\hat{\boldsymbol{\beta}}$ and $\hat{\boldsymbol{\tau}}$ are estimated, one can get an estimate for the mean, the log of variance and variance components for the observed values of $\mathbf{x}_i'$ and $\mathbf{z}_i'$ 
...
These can be returned by generic method `fitted()` which accepts the argument `type`. By default, `type = "all"` so all three quantities are estimated, but the user can choose to specify only `"mu"`, `"log.s2"` or `"s2"`.

```{r}
head(fitted(mod.mvreg))
```

The generic `predict()` is also available. It takes as argument the `mvreg` object, and if no other parameter is specified, its output is the same of `fitted()`, but the predictions are organized in a list of three different elements for `mu`, `log.s2` and `s2`.

```{r}
predict(mod.mvreg)
```

`predict()` takes a parameter `type` which works as in `fitted()`, but different components can be returned simultaneously.


It is possible to pass a `newdata` argument, which must be a `data.frame` containing the same variables used for fitting the `mvreg` model, both for mean and variance components
...
```
A further argument is `se.fit`, a logical value indicating whether to output an estimate of the standard error of the predictions. The standard errors for `mu` and `log.s2` are computed via standard formulas
...
while an estimate of standard error for $\hat{\sigma}^2_i$ is computed using Delta method
...
The argument `interval` is a logical value indicating whether to output a confidence interval for the predictions. The user can specify the significance level by the argument `sig.level`

```{r}
predict(mod.mvreg, newdata = newdata, se.fit = T, interval = T, sig.level = 0.95)
```

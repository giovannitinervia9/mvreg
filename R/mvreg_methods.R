...
summary.mvreg <- function(object, ...){
  ...
  mu.tab$`Pr(>|z|)` <- 2*pmin(pnorm(mu.tab$z.value, lower.tail = T),
  ...
  s2.tab$`Pr(>|z|)` <- 2*pmin(pnorm(s2.tab$z.value, lower.tail = T),
...
print.summary.mvreg <- function(x, digits = max(3L, getOption("digits") - 3L),
  ...
  printCoefmat(as.matrix(muMat), digits = digits, cs.ind = 1:2, tst.ind = 3,
  ...
  printCoefmat(as.matrix(s2Mat), digits = digits, cs.ind = 1:2, tst.ind = 3,
...
predict.mvreg <- function(object, type = c("all", "mu", "log.s2", "s2"), newdata, se.fit = F, interval = F, sig.level = 0.95){
  ...
  if(type == "all"){select <- c("mu", "log.s2", "s2")}
  else{select <- type}
  ...
  if(se.fit == F & interval == F){return(list(mu = pred.mu, log.s2 = pred.log.s2, s2 = pred.s2)[select])}

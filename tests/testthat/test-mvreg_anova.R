...
test_that("anova.mvreg accepts only mvreg objects", {
...
test_that("anova.mvreg performs single model comparison", {
  mod <- mvreg(Sepal.Length ~ Species + Sepal.Width, data = iris)
  result <- anova.mvreg(mod)
...
test_that("anova.mvreg performs multiple model comparison", {
  mod1 <- mvreg(Sepal.Length ~ Species + Sepal.Width, data = iris)
  mod2 <- mvreg(Sepal.Length ~ Species, ~1, data = iris)

  result <- anova.mvreg(mod1, mod2)
  ...
  expect_warning(anova.mvreg(mod1, mod1), "LRT test with models with the same number of parameters is meaningless")
})

test_that("anova.mvreg orders models correctly when order.models is TRUE", {
  mod1 <- mvreg(Sepal.Length ~ Species + Sepal.Width, data = iris)
  mod2 <- mvreg(Sepal.Length ~ Species, data = iris)

  result_ordered <- anova.mvreg(mod1, mod2, order.models = TRUE)
  params <- result_ordered$tests$n.param

  # Check that parameters are in ascending order
  expect_true(all(diff(params) >= 0))

  result_unordered <- suppressWarnings(anova.mvreg(mod1, mod2, order.models = FALSE))
  expect_warning(anova.mvreg(mod1, mod2, order.models = FALSE))
  # Ensure no reordering occurs
  expect_equal(rownames(result_unordered$tests), c("model1", "model2"))
})

test_that("anova.mvreg calculations are consistent", {
  mod1 <- mvreg(Sepal.Length ~ Species + Sepal.Width, data = iris)
  mod2 <- mvreg(Sepal.Length ~ Species, data = iris)
...
test_that("anova doesn't accept models with different responses", {
  ...
  expect_error(anova.mvreg(mod1, mod2), "Models have different response variables")
})
...
test_that("anova doesn't accept models with different number of observation", {
  ...
  expect_error(anova.mvreg(mod1, mod2), "Models are fitted with different numbers of observations")
})
...
# Test that the print.anova.mvreg function produces expected output for a single model
# Test that the print.anova.mvreg function produces expected output for a single model
test_that("print.anova.mvreg displays correctly for single model input", {
...
test_that("print.anova.mvreg displays correctly for multiple models comparison", {
...
test_that("print.anova.mvreg respects digits argument for output precision", {
...
test_that("print.anova.mvreg accepts additional arguments", {
  anova_result <- anova(mod3)
